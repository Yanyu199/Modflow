...
|---|---|---|---|---|
| 项目创建与保存 | 项目包含边界、坐标系、单位、网格、地层、packages、运行记录 | 已有 `modflow_project` v1.0、`geology_model` v1.0 和后端 project/geology 持久化；Flow/run 仍是前端 state | 部分实现 | 建立 flow/run schema；保存/打开后可无损复现同一 MF6 输入文件 |
...
| 自动化测试 | 单元、API、数值回归 | 已有 MF6 executable、workspace、Project Schema/API、Geology Schema/API/持久化、隔离和最小稳定流 benchmark 测试 | 部分实现 | 扩展到后端唯一网格、package 写入、更多标准模型 |
...
2. 已有项目级和地质模型 schema，但还没有正式 flow/run schema，仍无法保证保存/打开后复现同一 MF6 输入文件。
